...
func inspect_resource(path: String) -> void:
	lines.append("## `%s`" % path.trim_prefix("res://"))
	lines.append("")
	if not ResourceLoader.exists(path):
		lines.append("**RESOURCE NOT PRESENT IN THIS CHECKPOINT**")
		lines.append("")
		return
	var resource: Resource = load(path)
	if resource is Mesh:
		inspect_mesh(resource as Mesh, path)
		return
	if not resource is PackedScene:
		lines.append("Imported resource type: `%s`" % resource.get_class())
		lines.append("")
		return
...
func inspect_mesh(mesh: Mesh, path: String) -> void:
	var stats := _new_stats()
	stats["meshes"] = 1
	var aabb := mesh.get_aabb()
	stats["mesh_min"] = aabb.position
	stats["mesh_max"] = aabb.position + aabb.size
	lines.append("Direct OBJ Mesh resource: surfaces=%d" % mesh.get_surface_count())
	lines.append("- Mesh `%s`: AABB `(%.3f, %.3f, %.3f)`%s" % [path.get_file(), aabb.size.x, aabb.size.y, aabb.size.z, _tree_pack_classification(path, path.get_file())])
	append_stats(stats)
...
func append_stats(stats: Dictionary) -> void:
	lines.append("")
...
func walk(node: Node, stats: Dictionary, depth: int, parent_transform: Transform3D, source_path: String) -> void:
	...
	if node is MeshInstance3D:
		...
		for bone_index in skeleton.get_bone_count():
			bone_names.append(str(skeleton.get_bone_name(bone_index)))
		lines.append("%s- Skeleton `%s`: %d bones" % [indent, node.name, skeleton.get_bone_count()])
		lines.append("%s  Bones: %s" % [indent, ", ".join(bone_names)])

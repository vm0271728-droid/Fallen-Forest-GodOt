...
func _initialize() -> void:
	...
	print("Fallen Forest Locust rig: %d controlled bones ready." % _bone_indices.size())
...
func _process(delta: float) -> void:
	...
	for bone_name: String in _bone_indices.keys():
		var index: int = int(_bone_indices[bone_name])
		var current := skeleton.get_bone_pose_rotation(index)
		var target: Quaternion = _pose_targets.get(bone_name, Quaternion.IDENTITY)
...
func _reset_targets() -> void:
	for bone_name: String in _pose_targets.keys():
		_pose_targets[bone_name] = Quaternion.IDENTITY

func _apply_hide_pose(variant: int) -> void:
	# Five deliberately different silhouettes: 2 far, 1 medium, 2 close.
	var tiny_sway := sin(_time * 0.58 + float(variant)) * deg_to_rad(0.8)
	match variant:
		0: # far A — tall, one shoulder hanging lower
		...
		1: # far B — narrow hunched silhouette
		...
		3: # close A — lowered mass with long arm forward
		...
		4: # close B — opposite-side predatory crouch
...
func _apply_retreat_pose() -> void:
	_set_pose("spine.002", Vector3(deg_to_rad(6.0), 0.0, 0.0))
	_set_pose("upper_arm.L", Vector3(deg_to_rad(11.0), 0.0, deg_to_rad(5.0)))
	_set_pose("upper_arm.R", Vector3(deg_to_rad(11.0), 0.0, deg_to_rad(-5.0)))
...
func _apply_rage_pose() -> void:
	...
	_set_pose("upper_arm.L", Vector3(deg_to_rad(17.0), 0.0, 0.0))
	_set_pose("upper_arm.R", Vector3(deg_to_rad(17.0), 0.0, 0.0))

func _apply_arm_supported_chase() -> void:
	# Alternating long-arm contacts: both arms stay low/forward, one takes the
	# stronger support load while the opposite arm advances.
	...
	_set_pose("spine.001", Vector3(deg_to_rad(14.0) + torso_bob, 0.0, 0.0))
	_set_pose("spine.002", Vector3(deg_to_rad(10.0), 0.0, 0.0))
	_set_pose("upper_arm.L", Vector3(deg_to_rad(34.0 + left_contact * 12.0), deg_to_rad(-7.0), deg_to_rad(9.0)))
	_set_pose("forearm.L", Vector3(deg_to_rad(-31.0 - left_contact * 10.0), 0.0, deg_to_rad(-5.0)))
	_set_pose("hand.L", Vector3(deg_to_rad(8.0 + left_contact * 8.0), 0.0, 0.0))
	_set_pose("upper_arm.R", Vector3(deg_to_rad(34.0 + right_contact * 12.0), deg_to_rad(7.0), deg_to_rad(-9.0)))
	_set_pose("forearm.R", Vector3(deg_to_rad(-31.0 - right_contact * 10.0), 0.0, deg_to_rad(5.0)))
	_set_pose("hand.R", Vector3(deg_to_rad(8.0 + right_contact * 8.0), 0.0, 0.0))
